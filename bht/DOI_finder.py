...
def check_main_doi_exist(content):
    content = re.sub(".*<encodingDesc>\n[\s\S]+", "",
                     content)  # delete everything after <encodingDesc> markup in the xml file
...
def DOI_search_on_web(content):
    ...
    json_dict = os.popen(
        "curl --silent -H 'Authorization: Bearer:" + token + "' 'https://api.adsabs.harvard.edu/v1/search/query?q=" + title + "&fl=title,doi'| python -m json.tool").read()
    json_object = json.loads(json_dict)
    if json_object['response']['numFound'] != 0:
        return json_object['response']['docs'][0]['doi'][
            0]  # return the doi of the first result (already sorted by title similarity by ads)
    else:
        return 1
...
def find_DOI(file_path):
    ...
    else:  # if it doesn't exist, search doi on web with nasa ads api
