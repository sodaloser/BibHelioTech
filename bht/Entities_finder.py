...
def operating_span_checker(sat, durations, SAT_dict, SPAN_dict, published_date):
    # Checks that an interval linked to a satellite is included in the operating span of that sattelite.
    try:
...
def entities_finder(current_OCR_folder):
    ...
    for sat_1 in sat_dict_list:
        for inst_2 in inst_dict_list:
            if sat_1 != {}:
                if (inst_2['start'] - 1 <= sat_1['start']) and (inst_2['end'] + 1 >= sat_1['end']):  # is included
                    sat_1.clear()
                    # break
        temp.append(sat_1)
    ...
    # Change the names of all found satellites by their main name (AMDA name when existing OR first name in SAT_dict)
    for elements in final_links:
    ...
    # Association of the closest duration of a satellite. If it is not included in the satellite's operating span, search for the Nth closest duration.
        ...
        if dicts['type'] == 'sat':
            ...
            while sens_aller < len(temp):
                if temp[sens_aller]['type'] != 'sat':
                    ...
                    if operating_span_checker(dicts, temp[sens_aller], SAT_dict, SPAN_dict, published_date):
            ...
            while sens_retour >= 0:
                if temp[sens_retour]['type'] != 'sat':
                    ...
                    if operating_span_checker(dicts, temp[sens_retour], SAT_dict, SPAN_dict, published_date):
    ...
    for elements in final_links:
        ...
        elements[0]['conf'] = (elements[0]['D'] * elements[0]['R']) / (
                elements[0]['SO'] / TSO['occur_sat'])  # a normalisé par max de conf
    ...
    # SAT and REG linker
    # result and path should be outside the scope of find_path to persist values during recursive calls to the function
    path = []
    ...
    for elems in final_links:
        ...
        if len(temp_reg) == 0 or nearest_region is None:
            if elems[0]['text'] in AMDA_dict:
