...
for elems, val in SATS_dict.items():
    with open(os.path.join(papers_dir, elems+"_bibheliotech.txt"), "w") as f:
        ...
        f.write("# Description: Catalogue of events resulting from the HelioNER code (Dablanc & Génot, " + "\"https://github.com/ADablanc/BibHelioTech.git\"" + ") on the papers mentioning "+elems+" as listed in the third column by their DOI. The two first columns are the start/stop times of the event; the fourth column is the "+elems+" with the list of instruments (1 or more) listed in the fifth column. The sixth column is the most probable region of space where the observation took place (SPASE ObservedRegions term).\n")
